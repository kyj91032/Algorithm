# 다이내믹 프로그래밍: 큰 문제를 작은 문제로 나눌 수 있으며 작은 문제로 나눌 수 있으며 작은 문제의 답을 모아서 큰 문제를 해결할 수 있고, 동일한 작은 문제를 반복적으로 해결해야 할 때의 문제 해결 방법이다. (

# 다이내믹 프로그래밍의 기본 동작 과정

1. 완전 탐색을 시도했을 때, 작은 문제들이 반복적으로 등장하고 그 결과값을 재사용함으로써 큰 문제를 해결할 수 있다면. -> dp임.
(bf 중에 dp를 활용할 수 있는 구조인지 알 수 있음)
2. 어떤 걸 가지고 다음 i에 대한 d를 만들어낼 수 있는지를 생각한다. 이게 결국 코드를 만들어가는 과정.
