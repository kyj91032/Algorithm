...
1. dp임. -> 완전 탐색을 시도했을 때, 재귀적으로 얽힌 문제들이 반복적으로 등장하고 결과값을 재사용함으로써 큰 문제를 해결할 수 있다면.
(결국 구조를 보면 bf 중에 dp를 활용할 수 있을지 염두해둘 수 있음)
2. i가 추가됨에 따라 점화식이 이전 i에 대해 어떤 정보를 요구하는가를 파악하며 일반적인 i에 대한 점화식을 완성한다
