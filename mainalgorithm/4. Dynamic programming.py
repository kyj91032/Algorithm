...
1. dp임. -> 완전 탐색 시, 재귀적으로 연결된 작은 문제를 반복적으로 해결(결과값의 재사용 => dp 테이블 필요)함으로써 큰 문제를 해결할 수 있는가?를 확인.
2. 초기 점화식 세우기 (점화식이 이전 i에 대해 어떤 정보를 요구하는가?, 모든 경우가 고려되는가?, 점화식의 초깃값과 정복 방향은?). i와 di를 정의
3. 일반적인 i에 대한 구체적인 점화식을 구한다
