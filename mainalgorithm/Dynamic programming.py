# 다이내믹 프로그래밍: 큰 문제를 작은 문제로 나눌 수 있으며 작은 문제의 답을 모아서 큰 문제를 해결할 수 있고, 동일한 작은 문제를 반복적으로 해결해야 할 때의 문제 해결 방법이다.

# 다이내믹 프로그래밍의 기본 동작 과정

1. 완전 탐색을 시도했을 때, 작은 문제들이 반복적으로 등장하는 구조이며 그 결과값을 재사용함으로써 큰 문제를 해결할 수 있다면.(= 점화식이 세워진다면) -> dp임.
