...
# 재귀 함수의 기본 동작 과정
'''
1. 재귀 함수를 정의한다. (매개변수 설정)
2. 종료 조건(= 재귀 조건)을 설정하여 반복하는데, 이를 만족했을 때 함수가 종료되고 만족하지 못할 때 재귀 호출이 일어나도록 해야 한다.
재귀 조건의 경우 이를 만족했을 때 재귀 호출이 일어나고, 만족하지 못할 때 함수가 종료되도록 해야 한다.
