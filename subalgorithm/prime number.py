...
1. 2부터 sqrt(n)까지 모든 수를 확인하며
2. 약수가 존재하는지(n % i == 0) 판별
...
1. 소수 판별 테이블 True로 초기화 (결국 테이블이기 때문에 처음 실행 시 한번만 만들어 두면 됨. n을 받을 때마다 테이블을 생성하지 않아도 된다.)
2. 2부터 sqrt(n)까지 모든 수를 확인하며
3. 남은 수(소수) i를 제외한 i의 모든 배수를 지운다
